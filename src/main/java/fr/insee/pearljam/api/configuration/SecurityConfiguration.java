...
public class SecurityConfiguration {
	...
	@Value("${fr.insee.pearljam.application.mode}")
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		System.setProperty("keycloak.enabled", mode != SecurityMode.keycloak ? "false" : "true");
		http
		...
		if (mode == SecurityMode.basic) {
			...
			http.authorizeRequests().antMatchers(HttpMethod.OPTIONS).permitAll()
					.antMatchers("/swagger-ui/**", "/swagger-ui.html/**", "/v3/api-docs/**", "/csrf", "/",
							"/webjars/**",
							"/swagger-resources/**")
					.permitAll()
					// healtcheck
					...
					.hasAnyRole(adminRole, userLocalRole, userNationalRole)
					.antMatchers(HttpMethod.GET, Constants.API_USER)
					.hasAnyRole(adminRole, userLocalRole, userNationalRole)
	...
	public InMemoryUserDetailsManager userDetailsService() {
		...
		switch (mode) {
			case basic:
				UserDetails user1 = User.withUsername("INTW1").password("{noop}intw1")
						.roles(adminRole, interviewerRole).build();
				UserDetails user2 = User.withUsername("ABC").password("{noop}abc")
						.roles(adminRole, userLocalRole, userNationalRole).build();
				UserDetails user3 = User.withUsername("JKL").password("{noop}jkl")
						.roles(adminRole, userLocalRole, userNationalRole).build();
				UserDetails user4 = User.withUsername("noWrite").password("{noop}a").roles().build();

				inMemoryManager.createUser(user1);
				inMemoryManager.createUser(user2);
				inMemoryManager.createUser(user3);
				inMemoryManager.createUser(user4);
